...
if [ $NODE_TYPE -ne 2 ]; then
	MINE_OPTIONS="--mine --minerthreads $MINER_THREADS --etherbase $ETHBASE";
else
	FAST_SYNC="--fast";
fi
...
nohup geth --datadir $GETH_HOME --bootnodes $BOOTNODE_URLS --maxpeers $MAX_PEERS --nat none --networkid $NETWORK_ID --identity $IDENTITY $MINE_OPTIONS $FAST_SYNC --rpc --rpcaddr "$IPADDR" --rpccorsdomain "*" >> $GETH_LOG_FILE_PATH 2>&1 &
echo "===== Started geth =====";
